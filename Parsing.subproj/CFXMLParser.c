...
static CFStringRef __CFXMLParserCopyDescription(CFTypeRef cf) {
    ...
    return CFStringCreateWithFormat(CFGetAllocator(cf), NULL, CFSTR("<CFXMLParser 0x%x>"), parser);
}
...
__private_extern__ void __CFXMLParserInitialize(void) {
...
void CFXMLParserGetContext(CFXMLParserRef parser, CFXMLParserContext *context) {
	...
	context->retain = (void *)((uintptr_t)parser->context.retain & __mask);
	context->release = (void *)((uintptr_t)parser->context.release & __mask);
	context->copyDescription = (void *)((uintptr_t)parser->context.copyDescription & __mask);
    }
}
...
void CFXMLParserGetCallBacks(CFXMLParserRef parser, CFXMLParserCallBacks *callBacks) {
    ...
    if (callBacks) {
        ...
        callBacks->createXMLStructure = (void *)((uintptr_t)parser->callBacks.createXMLStructure & __mask);
        callBacks->addChild = (void *)((uintptr_t)parser->callBacks.addChild & __mask);
        callBacks->endXMLStructure = (void *)((uintptr_t)parser->callBacks.endXMLStructure & __mask);
        callBacks->resolveExternalEntity = (void *)((uintptr_t)parser->callBacks.resolveExternalEntity & __mask);
        callBacks->handleError = (void *)((uintptr_t)parser->callBacks.handleError & __mask);
    }
}
...
static CFXMLParserRef __CFXMLParserInit(CFAllocatorRef alloc, CFURLRef dataSource, CFOptionFlags options, CFDataRef xmlData, CFIndex version, CFXMLParserCallBacks *callBacks, CFXMLParserContext *context) {
    struct __CFXMLParser *parser = (struct __CFXMLParser *)_CFRuntimeCreateInstance(alloc, __kCFXMLParserTypeID, sizeof(struct __CFXMLParser) - sizeof(CFRuntimeBase), NULL);
    ...
    if (parser && node) {
        ...
        buf = CFAllocatorAllocate(alloc, 128*sizeof(UniChar), 0);
        parser->node = node;
...
Boolean CFXMLParserParse(CFXMLParserRef parser) {
    ...
    // Create the document
    parser->stack = CFAllocatorAllocate(CFGetAllocator(parser), 16 * sizeof(void *), 0);
    parser->capacity = 16;
        ...
        if (attributes) {
            if (capacity == attListData.numberOfAttributes) {
                ...
                if (attributes != attributeArray) {
                    attributes = CFAllocatorReallocate(CFGetAllocator(parser), attributes, capacity * sizeof(CFXMLAttributeDeclarationInfo), 0);
                } else {
                    attributes = CFAllocatorAllocate(CFGetAllocator(parser), capacity * sizeof(CFXMLAttributeDeclarationInfo), 0);
                }
            }
...
static void pushXMLNode(CFXMLParserRef parser, void *node) {
    ...
    if ((unsigned)(parser->top - parser->stack) == parser->capacity) {
        parser->stack = CFAllocatorReallocate(CFGetAllocator(parser), parser->stack, 2 * parser->capacity * sizeof(void *), 0);
    ...
    for(i = 0; i < length; ) {
        ...
        if(uc == '&') {
            ...
            if(0xFFFF != entity) { // it was numeric, and translated.
                ...
                CFStringRef replacementString = CFDictionaryGetValue(fullReplDict, sub);
